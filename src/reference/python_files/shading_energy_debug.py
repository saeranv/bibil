    ...
    if numOfShds == 0 or distBetween == 0:
        ...
        if normalVector.X == 0 and normalVector.Y == 0:
            ...
            try:
                numOfShd = int(numOfShds)
                # edit shading height
                shadingHeight = glzHeight/(numOfShd-1)
                shadingRemainder = shadingHeight
            except:
                shadingHeight = distBetween
                shadingRemainder = (((glzHeight/distBetween) - math.floor(glzHeight/distBetween))*distBetween)
            ...
            if divisionParams == None:
                divisionParams = [0.0]
            divisionParams = list(divisionParams)
            if len(divisionParams) < numOfShd:
                divisionParams.append(divisionParams[-1] + shadingHeight)
            ###
            print divisionParams
            print 'num of shade', numOfShd
            print shadingHeight
            print ''
            ###
